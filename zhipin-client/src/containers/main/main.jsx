...
class Main extends Component{
    ...
    componentDidMount () {
        //登录过(cookie中右userid)，但还没登录(redux管理的user中没有_id) 发送请求获取对应的user，暂时不做任何显示
    ...
    render () {
        ...
        //如果有user没有_id,返回null(不做任何显示)
        if(!user._id) {
            return null
        } else {
            //如果有_id，显示对应的界面
            //如果请求根路径,根据user的type和header来计算出一个重定向的路径，并自动重定向
...
1实现自动登入
    1.componentDidMount()
        登录过(cookie中右userid)，但还没登录(redux管理的user中没有_id) 发送请求获取对应的user
    2.render()
        1).如果cookie中没有userid，直接重定向到login
        2).判断redux管理的user中是否有_id，如果没有，暂时不做任何显示
        3).如果有，说明当前已经登录显示对应的界面
        4).如果请求根路径：根据user的type和header来计算出一个重定向的路径，并自动重定向
*/